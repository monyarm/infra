...
{
  programs.mpv.scriptOpts.dvd_browser = {
    #################################################################
    ...
    dvd_device = lib.mkIf (!pkgs.stdenv.isDarwin) "/dev/sr0"; # Assuming Android is not Darwin, and /dev/sr0 is the default for Linux
