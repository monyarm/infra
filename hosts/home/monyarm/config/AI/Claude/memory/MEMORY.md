...
- [Never restore uncommitted work](feedback_never_restore_uncommitted_work.md) — never use `git restore`, `git checkout`, or `git reset` as a recovery shortcut; the index is not a backup, and autogenerated `sources.nix` must not be edited directly.
