...
in
{
  home.activation.flatpakManagement = lib.mkIf shouldFullUpdate (
    lib.hm.dag.entryAfter [ "linkGeneration" ] ''
      ...
      ${dirs.lib}/flatpak_remote_manager.sh "flathub" "https://flathub.org/repo/flathub.flatpakrepo"

      # 1.1. Ensure the Flathub Beta repo is added
      ${dirs.lib}/flatpak_remote_manager.sh "flathub-beta" "https://flathub.org/beta-repo/flathub-beta.flatpakrepo"
      ...
      ${lib.concatStringsSep "\n" (
        lib.map (flatpak: ''
          ${dirs.lib}/flatpak_program_manager.sh "${flatpak}" || true
        '') desiredFlatpaks
      )}
