...
rec {
  ...
  #   Type: cropImage -> { src, aspectRatio, ?gravity } -> Derivation
  cropImage =
    {
      src,
      aspectRatio,
    ...
    let
      # Use lib.getName to robustly extract the filename from the derivation.
      # This handles hash prefixes correctly.
      fullFileName = lib.getName src;

      # Determine baseName and extension more robustly
      fileNameParts = lib.splitString "." fullFileName;
      baseName =
        if lib.length fileNameParts > 1 then
          lib.concatStringsSep "." (lib.init fileNameParts)
        else
          fullFileName; # No dot found, treat entire name as baseName
      extension = if lib.length fileNameParts > 1 then lib.last fileNameParts else "png"; # Default to png if no extension found

      name = "${baseName}-cropped-${aspectRatio}.${extension}";
    in
    pkgs.runCommand name
      ...
      ''
        convert ${src} -gravity ${gravity} -crop "${aspectRatio}" -format png +repage $out
      '';

  # crop16x9:
  #   Crops an image to a 16:9 aspect ratio using ImageMagick.
  #   Type: crop16x9 -> (String | { src, ?gravity }) -> Derivation
  crop16x9 =
    args:
    let
      src = if lib.isString args then args else args.src;
      gravity = if lib.isAttrs args && args ? gravity then args.gravity else "center";
    in
    cropImage {
      inherit src gravity;
      aspectRatio = "16:9";
