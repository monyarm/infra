...
# Usage: prefetch_curseforge.sh --modId <id> --fileId <id> [--output-file <file>]
# Requires CURSEFORGE_API_KEY to be available to the build sandbox via /secrets.
...
if [[ -z ${modId:-} || -z ${fileId:-} ]]; then
  echo "Missing --modId or --fileId argument" >&2
  exit 1
fi
